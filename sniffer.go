...
func handleProtoPacket(data []byte, fromServer bool, timestamp time.Time) {
	...
	if config.AutoFindMinimalProto {
		packetCounter++
		data2 := removeMagic(data)
		if packetCounter == 2 {
			log.Println("second packet, assuming PlayerGetTokenScRsp")
			possibleSeed, err := unkPlayerGetTokenScRsp(data2)
			if err == nil {
				deducedPacketIds[packetId] = "PlayerGetTokenScRsp"
				packetIdMap[packetId] = "PlayerGetTokenScRsp"
				packetNameMap["PlayerGetTokenScRsp"] = packetId
				sessionKey = createXorPad(possibleSeed)
			} else {
			...
			if err == nil {
				...
				deducedPacketIds[packetId] = "GetQuestDataScRsp"
				packetIdMap[packetId] = "GetQuestDataScRsp"
				packetNameMap["GetQuestDataScRsp"] = packetId
	...
	if packetId == playerGetTokenScRspPacketId && !config.AutoFindMinimalProto {
